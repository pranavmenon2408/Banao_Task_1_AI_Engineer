...
JD = (
    "We are hiring a Senior Backend Engineer. Requirements: 5+ years of Python, PostgreSQL, Kafka, Docker and AWS. "
...
def test_jd_both_or_neither(client, resume_lines):
    ...
    r = client.post(
        "/api/v1/score", files={"resume": cv, "jd_file": ("jd.txt", JD.encode(), "text/plain")}, data={"jd_text": JD}
    )
    assert r.status_code == 422
...
def test_rescore_reweights_without_llm(client):
    ...
    other = {
        **crit,
        "criterion": {**crit["criterion"], "id": "c2", "name": "K8s", "importance": "nice_to_have"},
        "raw_level": 0,
        "final_level": 0,
    }
    base = client.post("/api/v1/rescore", json={"criteria": [crit, other], "overrides": {}}).json()
    zeroed = client.post(
        "/api/v1/rescore", json={"criteria": [crit, other], "overrides": {"importance_weights": {"nice_to_have": 0}}}
    ).json()
