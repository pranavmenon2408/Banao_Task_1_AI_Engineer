...
Many resume builders draw a two-column layout (sidebar + main column) row by row across the page.
A reader that follows content-stream order (pypdf did) then interleaves the columns line by line:
"...transaction status / Kafka / updates, cutting reconciliation lag..." (observed, docs/DEVLOG.md).

PyMuPDF gives every text line with its bounding box, so we can decide reading order ourselves:
  1. Find a vertical gutter: an x-range no body line crosses, with substantial text on both sides.
     "Substantial" (>= 8% of the characters and >= 5 lines per side) stops a right-aligned column of
     dates from being mistaken for a second column and torn away from its roles.
  2. Lines crossing the gutter (a full-width name/header) split the page into bands.
  3. Within each band emit the left column top-to-bottom, then the right column.
Single-column pages have no qualifying gutter and are read top-to-bottom as usual.
"""
...
def find_gutter(lines: list[Line], width: float) -> float | None:
    ...
    for g0, g1 in sorted(gaps, key=lambda g: g[1] - g[0], reverse=True):
        ...
        # In a real two-column page only a header or two spans the gutter. If many lines cross it
        # (e.g. full-width bullets with right-aligned dates on the role lines) it is one column.
        if (min(len(left), len(right)) >= SIDE_MIN_LINES
                and lc >= SIDE_MIN_SHARE * total and rc >= SIDE_MIN_SHARE * total
