"""Provider-agnostic LLM client with retries, fallback models and validated JSON output.

The pipeline makes a small number of fixed LLM calls with no tools, memory or routing, so a thin client is used
instead of an orchestration framework: every prompt, retry and parse step stays explicit and testable.

Error handling, per model and then down the fallback list:

* transient failures (timeouts, 429, 5xx, network)  -> exponential backoff, then the next model
* model not served by the provider                   -> next model; if none is served, MODEL_NOT_AVAILABLE
* invalid or missing API key                          -> CONFIG_ERROR immediately
* provider rejects `response_format`                  -> prompt-only JSON from then on
* output is not valid JSON for the schema             -> one repair turn showing the model its error
* every model exhausted                               -> LLM_UNAVAILABLE
"""
...
from app.llm.providers import Messages, ProviderError, Transport, build_transport, get_provider
...
class LLMError(Exception):
    """An LLM call failed; `code` tells the API which error to return."""
...
def _split(models: str | list[str] | tuple[str, ...] | None) -> list[str]:
    """Normalise a comma-separated string or list of model names."""
    if not models:
        return []
...
class LLMClient:
    """One interface over any provider, with retries and fallback models.

    Examples:
        LLMClient()                                        # text model from settings (LLM_PROVIDER / LLM_MODEL)
        LLMClient(provider="openai", model="gpt-4o-mini")  # explicit provider and model
        LLMClient(role="vlm")                              # vision model from settings (VLM_*)
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.provider_name = get_provider(
            provider or (s.vlm_provider if vlm and s.vlm_provider else s.llm_provider)
        ).name
        self.model = model or (s.vlm_model if vlm else s.llm_model)
        fb = (
            fallback_models
            if fallback_models is not None
            else (s.vlm_fallback_models if vlm else s.llm_fallback_models)
        )
    ...
    def transport(self) -> Transport:
        """The provider transport, built lazily so a missing key surfaces as an LLMError on first use."""
        with self._lock:
            if self._transport is None:
                try:
                    self._transport = build_transport(
                        get_provider(self.provider_name), self._api_key or "", self._endpoint or "", self.hf_provider
                    )
    ...
    def _call(
        ...
        """Send messages through the model list and return the first successful reply's text."""
            ...
            while True:
                try:
                    ...
                    reply = self.transport.send(
                        ...
                        temperature=temp,
                        seed=seed,
                        json_mode=json_mode and self._json_mode,
                    ...
                    if exc.kind == "auth":
                        raise LLMError(
                            ...
                            "Check the key variable for this provider in .env.",
                        ) from exc
                    ...
                    if exc.kind == "transient" and attempt < self.s.llm_max_retries:
                        ...
                        log.warning(
                            "Transient LLM error on %s (%s); retry %d in %ss",
                            model,
                            exc.message[:120],
                            attempt + 1,
                            delay,
                    ...
                    log.warning(
                        "Model %s failed (%s: %s); trying next model if any", model, exc.kind, exc.message[:160]
                    )
                    break
    ...
    def complete_json(
        ...
        """Return the model's reply parsed and validated as `schema`, with one repair turn on invalid output.

        Raises:
            LLMError: the call failed or the output was invalid twice.
        """
        ...
        try:
            ...
            try:
                ...
                raise LLMError(
                    ErrorCode.LLM_BAD_OUTPUT, f"LLM returned invalid output twice: {str(exc)[:300]}"
                ) from exc

    def complete_text(self, messages: Messages, metric: StageMetric, max_tokens: int = 2000) -> str:
        """Plain-text completion at temperature 0, with the same retries and fallbacks as `complete_json`."""
