"""Turn per-criterion rubric levels into the final score. Pure code, no LLM: fully determined by
config/scoring.yaml, so changing weights never changes the model's judgements, and the arithmetic is
reproducible.

    weight_i  = importance_weights[importance_i] * category_weights[category_i]
    points_i  = level_points[final_level_i]          (final = raw level minus grounding penalty)
    overall   = sum(weight_i * points_i) / sum(weight_i)
    if knockout.enabled and any must_have scored 0: overall = min(overall, knockout.cap)
"""
...
def build_scored(criterion: Criterion, a: CriterionAssessment, evidence: list[EvidenceCheck], jd_grounded: bool,
                 cfg: ScoringConfig) -> ScoredCriterion:
    flags: list[str] = []
    grounded = a.level == 0 or any(e.found for e in evidence)
    final = a.level
    if not grounded:
        final = max(0, a.level - cfg.grounding.ungrounded_level_penalty)
        flags.append("Evidence quote not found in resume; level reduced" if evidence
...
def aggregate(scored: list[ScoredCriterion], cfg: ScoringConfig,
              overrides: WeightOverrides | None = None) -> tuple[float, str, bool, dict]:
    ...
    knockout = cfg.knockout.enabled and any(s.criterion.importance == "must_have" and s.final_level == 0 for s in scored)
    if knockout:
        overall = min(overall, cfg.knockout.cap)
    overall = round(overall, 1)
    label = next(b.label for b in cfg.recommendation_bands if overall >= b.min)
    return overall, label, knockout, {"importance_weights": imp, "category_weights": cat,
                                      "level_points": cfg.level_points, "knockout": cfg.knockout.model_dump()}


def strengths_and_gaps(scored: list[ScoredCriterion]) -> tuple[list[str], list[str]]:
    strengths = [f"{s.criterion.name} (level {s.final_level}/4)" for s in
                 sorted(scored, key=lambda s: (-s.final_level, -s.weight)) if s.final_level >= 3][:5]
    gaps = [f"{s.criterion.name}: {s.gaps or 'no evidence in resume'}" for s in
            sorted(scored, key=lambda s: (s.final_level, -s.weight))
            if s.final_level <= 1 and s.criterion.importance != "nice_to_have"][:5]
