"""Streamlit front end for the Resume-JD Fit Scorer. Talks to the FastAPI backend over HTTP only,
so the UI and API can be deployed and scaled separately (see docker-compose.yml)."""
...
IMPORTANCE = {"must_have": ("Must have", "#b42318", "#fef3f2"),
...
def show_error(resp: requests.Response | None, exc: Exception | None = None):
    if exc is not None:
    ...
    if cfg:
        with st.expander("Importance weights", expanded=True):
            for k, v in cfg["importance_weights"].items():
                overrides["importance_weights"][k] = st.slider(IMPORTANCE.get(k, (k,))[0], 0.0, 5.0, float(v), 0.5, key=f"iw_{k}")
        with st.expander("Category weights"):
            for k, v in cfg["category_weights"].items():
                overrides["category_weights"][k] = st.slider(k.replace("_", " ").title(), 0.0, 3.0, float(v), 0.1, key=f"cw_{k}")
        changed = (overrides["importance_weights"] != cfg["importance_weights"]
                   or overrides["category_weights"] != cfg["category_weights"])
    else:
        changed = False

    st.markdown("### ℹ️ How scores work")
    st.caption("Each criterion gets a rubric level 0-4 from the scorer, which must quote the resume. "
...
with c1:
    ...
    if mode == "Upload file":
        jd_file = st.file_uploader("JD file", type=["pdf", "docx", "txt"], key="jd_file", label_visibility="collapsed",
                                   help="PDF, DOCX or TXT")
    else:
        jd_text = st.text_area("JD text", height=210, placeholder="Paste the full job description here…",
                               label_visibility="collapsed")
        if jd_text:
            st.caption(f"{len(jd_text):,} characters")
with c2:
    st.markdown("#### 👤 Resume")
    resume = st.file_uploader("Resume file", type=["pdf", "docx", "txt"], key="resume", label_visibility="collapsed",
...
if go:
    ...
    with st.status("Assessing fit…", expanded=True) as status:
        ...
        status.update(label=f"Done in {r.json()['meta']['total_latency_ms'] / 1000:.1f}s", state="complete", expanded=False)
...
if changed and st.session_state.get("base_criteria"):
    rr = requests.post(f"{API_URL}/api/v1/rescore", json={"criteria": st.session_state["base_criteria"],
                                                           "overrides": overrides}, timeout=15)
    if rr.ok:
        res = {**res, **rr.json()}
...
for col, (val, label) in zip(k, [(f"{must_met}/{len(must)}", "Must-haves met"),
    ...
    col.markdown(f"<div class='card' style='text-align:center'><div class='kpi'>{val}</div><div class='kpi-l'>{label}</div></div>",
...
if res["knockout_triggered"]:
    st.warning(f"A must-have criterion has no evidence, so the overall score is capped at {res['weights_used']['knockout']['cap']}.")
...
with tab1:
    ...
    f1.caption("Each score cites the JD requirement it came from and the resume text that supports it. "
    ...
    for c in sorted(res["criteria"], key=lambda c: (order[c["criterion"]["importance"]], c["criterion"]["id"])):
        ...
        lvl = c["final_level"]
        bars = "".join(f"<span style='background:{LEVEL_COLOR[lvl]}'></span>" if i < lvl else "<span></span>" for i in range(4))
        ...
        jd_quote = (f"<div class='quote jdq'><span class='muted'>JD says:</span> “{esc(cr['jd_evidence'])}”</div>"
                    if cr.get("jd_evidence") else "")
        ev = "".join(
            f"<div class='quote {'ok' if e['found'] else 'bad'}'>{'✓' if e['found'] else '✗'} “{esc(e['quote'])}”"
            f" <span class='muted'>({e['similarity']:.0%} match)</span></div>" for e in c["evidence"])
        if not c["evidence"]:
...
with tab2:
    ...
    with a:
        ...
        st.markdown("**Skills**")
        st.markdown(" ".join(f"<span class='pill' style='background:rgba(128,128,128,.12);margin-bottom:6px'>{esc(s)}</span>"
                             for s in p.get("skills", [])), unsafe_allow_html=True)
        if p.get("education"):
            st.markdown("**Education**")
            for e in p["education"]:
                st.markdown("- " + ", ".join(esc(x) for x in (e.get("degree"), e.get("field"), e.get("institution"), e.get("year")) if x))
...
st.download_button("⬇️ Download full assessment (JSON)", json.dumps(res, indent=2),
                   file_name=f"fit_assessment_{res['meta']['run_id']}.json", mime="application/json")
