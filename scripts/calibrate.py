...
def main() -> None:
    ...
    for rep in range(args.repeats):
        for label, path in SAMPLES.items():
            ...
            try:
                ...
                continue
            row = {"overall": r.overall_score, "recommendation": r.recommendation,
                   ...
                   "n_criteria": len(r.criteria), "latency_s": r.meta.total_latency_ms / 1000,
            ...
            print(f"[{label}] run {rep + 1}: {r.overall_score:5.1f} {r.recommendation:14s} "
                  f"criteria={row['n_criteria']} ungrounded={row['ungrounded']} {row['latency_s']:.1f}s {row['tokens']} tok")
    ...
    for label, rs in runs.items():
        ...
        if not scores:
            continue
        summary[label] = {"mean": round(st.mean(scores), 1), "min": min(scores), "max": max(scores),
    ...
    for label, s in summary.items():
        lines.append(f"| {label} | {s['mean']} | {s['min']} | {s['max']} | {s['range']} | {s['stdev']} | "
